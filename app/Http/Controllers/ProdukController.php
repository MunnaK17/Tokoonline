    ...
    public function create()
    {
       $kategori = Kategori::orderBy('nama_kategori', 'asc')->get();
        return view('backend.v_produk.create', [
            'judul' => 'Tambah Produk', 'kategori' => $kategori
            ]); 
    }
    ...
    public function store(Request $request)
{
    ...
    // TAMBAHKAN INI: Agar error user_id hilang
    ...
    // Perbaikan: Hapus parameter $messages
    ...
    public function destroy($id)
{
    ...
    foreach ($produk->gambar as $gambar) {
        $additionalImagePath = $directory . $gambar->foto;
...
public function foto($filename)
{
    ...
    $imagePath = public_path('storage/img-produk/' . $safeFilename);

    if (is_file($imagePath)) {
        return Response::file($imagePath);
    }

    return Response::file(public_path('frontend/img/product01.jpg'));
}

public function produkKategori($id)
{
    $selectedKategori = Kategori::findOrFail($id);
    $kategori = Kategori::orderBy('nama_kategori', 'desc')->get();
...
{
    $kategori = Kategori::orderBy('nama_kategori', 'desc')->get();
    $produk = Produk::with('kategori')->where('status', 1)->orderBy('updated_at', 'desc')->paginate(9);
